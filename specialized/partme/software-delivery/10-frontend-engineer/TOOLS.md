...
### 默认推荐：`frontend-design`

前端工程师智能体在实现**需较高视觉层次与界面品质**的页面（营销页、控制台、落地页等）时，应加载并遵循 **`frontend-design`** 技能：

- **full-stack-skills 路径：** 插件类别 `development-skills-utils` → 技能名 `frontend-design`（见仓库 `skills/dev-utils-skills/frontend-design/SKILL.md`）。
- **skills.sh 等价：** `npx skills add anthropics/skills --skill frontend-design -y -g`（与 TOOLS.md 下文「首选」一致）。
- **OpenClaw：** 若使用 `agents.list[].skills` 显式绑定，请包含字符串 `frontend-design`。
